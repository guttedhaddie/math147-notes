...
\begin{thm}[lower separated=false, sidebyside, sidebyside align=top seam, sidebyside gap=0pt, righthand width=0.25\linewidth]{}{absconv}
...
We continue Example \ref{ex:geomseries}. On the disk $\nm z<1$, the function $f(z)=\frac 1{1-z}$ has 
  	...
  	The Maclaurin series is therefore the geometric power series representation of $f(z)$ on the open disk $\nm z<1$! %In accordance with the above observation \ref{obs:dist}, its radius of convergence $R_0=1$ is the distance from the center 0 to the nearest point $z=1$ at which $f$ fails to be differentiable.
...
%   	\item On the open disk $\nm z<1$, the function $f(z)=\frac 1{1-z}$ has 
%   	\[f^{(n)}(z)=\frac{n!}{(1-z)^{n+1}}\implies \frac 1{1-z}=\sum\limits_{n=0}^\infty\frac{f^{(n)}(0)}{n!}z^n =\sum\limits_{n=0}^\infty z^n\]
%   	The Maclaurin series is therefore the geometric series of Example \ref{ex:geomseries}; its radius of convergence is plainly $R_0=1$. In accordance with the above observation \ref{obs:dist}, this is the distance from the center 0 to the nearest point $z=1$ at which $f$ fails to be differentiable.
%   \end{minipage}\begin{minipage}[t]{0.3\linewidth}\vspace{-15pt}
%   \flushright\includegraphics{taylorex1}
%   \end{minipage}
% 
%   
