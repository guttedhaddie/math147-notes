...
Since limits and continuity behave similarly to real analysis, this section is mostly for reference and we omit most proofs. If you're only familiar with \emph{single-variable} real analysis, note how our first definition generalizes the notion of an open interval.
...
\begin{example}[lower separated=false, sidebyside, sidebyside align=top seam, sidebyside gap=0pt, righthand width=0.3\linewidth]{}{easyopen}
...
These limits have the same interpretation as Definition \ref{defn:limit}: $z$ `close to' $z_0$ means $f(z)$ is `close to' $w_0$. The difficulty is merely about the meaning of `close to $\infty$.' 
...
\begin{proof}[Sketch Proof]
	All six results are similar; we prove only the first.\smallbreak 
	Suppose $\smash{\lim\limits_{z\to\infty}f(z)=w_0}$. Let $\delta=\frac 1N$ to see that
	\[
		\forall \epsilon>0,\exists \delta>0\text{ such that }0<\tfrac 1z<\delta \implies \nm{f(\tfrac 1z)-w_0}<\epsilon \tag*{\qedhere}
	\]
...
\footnotetext{Loosely speaking, a connected set consists of one `lump.' For topologists: almost all domains of interest in these notes will be open, so there is no benefit to distinguishing between connectedness and path-connectedness.}
...
\begin{description}
	...
	\]
\end{description}
%If $f'(z)$ exists, then these limits are necessarily equal (to $f'(z)$ itself!). We have proved a key result:
...
\subsection{Holomorphic and Harmonic Functions}\label{subsec:analytic}% (\S\,24--28)
