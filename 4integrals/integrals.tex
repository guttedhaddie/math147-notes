...
The obvious calculus laws are easily verified by considering real and imaginary parts separately.
...
%\subsection[Contour Integrals]{Contour Integrals}\label{sec:contour}
...
This is often written $\oint_Cf$ if $C$ is positively oriented (simple and closed).%\smallbreak
% If the value $\int_Cf$ depends only on the endpoints of $C$ and is otherwise \emph{independent of path,} we may write $\int_{z_1}^{z_2}f(z)\,\dz$ where $z_1,z_2$ are the endpoints of $C$.
\end{defn}
...
\textcolor{red}{Warning!} Things are more difficult if we permit a contour to \emph{cross} a branch cut since we must then work with multiple branches simultaneously. We won't do this.
\goodbreak
...
\subsection{Path-independence, the Fundamental Theorem \& Integral Estimation}
...
% \\
% &=\left(-\frac 13-1\right)-\left(\frac 13+1\right)=-\frac 83
\end{align*}
...
% \begin{defn}{}{}
% A function $f:D\to\C$ has an \emph{anti-derivative} $F:D\to\C$ if $F'(z)=f(z)$ on $D$.
% \end{defn}
% 
% Recall our discussion of holomorphic functions, where we saw that the only functions with zero derivative on an open connected domain are the \emph{constants.} This immediately forces an intuitive result.
% 
% \begin{lemm}{}{}
% Suppose $F$ and $G$ are anti-derivatives of $f$ on an open connected domain $D$. Then $G(z)-F(z)$ is constant on $D$.
% \end{lemm}
% 
% This shows that we could have used \emph{any} anti-derivative $\frac 13z^3+z+k$ to evaluate the previous example.\bigbreak

%We can now state an initial version of a powerful result relating anti-derivatives and the path-independence of contour integrals.
% 
% \begin{lemm}{Fundamental Theorem of Line Integrals}{fundthm}
% If $F$ is an anti-derivative of $f$ on $D$ and $C$ is any contour within $D$ and with start/end points $z_s,z_e$, then the contour integral
% \[\int_Cf=F(z_e)-F(z_s)\]
% is path-independent. Moreover, if $C$ is additionally closed, then
% $\int_C=0$
% \end{lemm}

% \begin{proof}
% The result is already true for every smooth arc by Lemma \ref{lemm:chainsubs}. Now let $C=C_1\cup\cdots\cup C_n$ be a contour where the smooth arcs $C_k$ are arranged in order and have start/end points $z_{k-1},z_k$. By Theorem \ref{thm:basiccontour}, part 1, we see that
% \[\int_Cf=\sum_{k=1}^n\int_{C_k}f =\sum_{k=1}^n\left[F(z_k)-F(z_{k-1})\right] =F(z_n)-F(z_0)\tag*{\qedhere}\]
% \end{proof}
% \goodbreak

Since the proof requires a little work, we postpone it until after some examples.
...
\boldsubsubsection{Proving the Fundamental Theorem}

The forward direction is relatively straightforward. Before reading the converse however, you may find it helpful to review part I of the fundamental theorem from real analysis, namely
...
%  subtleties:
% \begin{itemize}
%   \item We must choose how to travel from one endpoint of an integral to the other: path independence says it doesn't matter how.
%   \item We'll need to bound the modulus of a contour integral: this isn't as trivial as it may seem\ldots
% \end{itemize}
...
\begin{enumerate}\setcounter{enumi}{1}
  ...
  \item\begin{enumerate}
    \item Find an anti-derivative of $\frac 1{1-z^2}$ on the domain $D=\C$ except for the real axis where $\nm z\ge 1$. Evaluate $\int_0^{1+2i}\frac 1{1-z^2}\,\dz$ along any curve in $D$.
	
	\begin{minipage}[t]{0.77\linewidth}\vspace{0pt}
    \item Prove directly that $\oint_{C_r}\frac 1{1-z^2}\,\dz =-i\pi$ where $C_r$ is the circle of radius $r<2$ centered at $z=1$.\par
    (\emph{Hint: use the anti-derivative and the circle starting and finishing on the branch cut at $z=1+r$})
    
    \item Evaluate $\int_0^{1+2i}\frac 1{1-z^2}\,\dz$ along a curve $C_2$ looping to the \emph{right} of $z=1$ as shown in the picture. Compare your answer with parts (a) and (b).
	\end{minipage}\begin{minipage}[t]{0.23\linewidth}\vspace{0pt}
...
\begin{enumerate}\setcounter{enumi}{1}
  ...
  \begin{itemize}
    \item If the origin is \emph{exterior} to $C$, then $f(z)$ is holomorphic on and inside $C$, whence $\oint_C\frac 1z\,\dz=0$.
    
  \begin{minipage}[t]{0.6\linewidth}\vspace{0pt}
    \item If the origin is \emph{interior} to $C$, then there is some minimum distance $d$ of $C$ to the origin. Choose any circle $C_r$ with radius $r<d$ centered at the origin. Since $f(z)$ is holomorphic on the region $D$ between and including $C$ and $C_r$, we conclude that
\[\oint_C\frac 1z\,\dz=\oint_{C_r}\frac 1z\,\dz =\int_0^{2\pi}\frac 1{re^{i\theta}}rie^{i\theta}\,\dth=2\pi i\]
  \end{minipage}
  \begin{minipage}[t]{0.39\linewidth}\vspace{0pt}
  \flushright\includegraphics{cauchygoursat}
  \end{minipage}
  \end{itemize}
...
\begin{proof}[Proof of the basic Integral formula]
...
%$\partials{z_0}\frac{n!}{(z-z_0)^{n+1}}=\frac{(n+1)!}{(z-z_0)^{n+2}}$
...
Finally, note that if $f$ has an anti-derivative $F$, then $F$ is necessarily holomorphic and so, by the Corollary, is $f$ itself. Combining our results yields the following.\smallbreak
...
%   \item Suppose we attempted to prove Cauchy's integral formula $f(z_0)=\frac 1{2\pi i}\oint_C\frac{f(z)}{z-z_0}\,\dz$ purely for \emph{circular paths} $C$. Explain why we still need the full Cauchy--Goursat Theorem to complete the proof.\\[5pt]
%   \emph{If we didn't, then the full version would be redundant: Corollary \ref{cor:infdiff} would show that every holomorphic function $f(z)=u+iv$ had continuous partial derivatives $u_x,u_y,v_x,v_y$, whence our sketch proof of Cauchy--Goursat (Theorem \ref{thm:cauchygoursatv1}) relying on Green's Theorem would be enough to establish the complete integral formula\ldots}
\end{enumerate}
\end{exercises*}
...
% 
% 
% Now for the extension. As above, let $\delta>0$ be such that $\nm{w-z_0}<\delta\implies w\in D$, and let $\nm{\Delta z}<\delta$. For any $z\in C$, we have
% \[\nm{z-z_0}\ge\delta,\quad \nm{z-z_0-\Delta z}\ge \bigl|\nm{z-z_0}-\nm{\Delta z}\bigr|\ge \delta-\nm{\Delta z}>0\] 
% Then
% \begin{gather*}
% \nm{\frac{f(z_0+\Delta z)-f(z_0)}{\Delta z}-\frac 1{2\pi i}\oint_C\frac{f(z)}{(z-z_0)^2}\,\dz}\\
% \begin{aligned}
% \qquad\qquad&=\nm{\frac 1{2\pi i}\oint_C\frac{f(z)}{\Delta z}\left(\frac 1{z-z_0-\Delta z}-\frac 1{z-z_0}\right)-\frac{f(z)}{(z-z_0)^2}\,\dz}\\
% &=\frac 1{2\pi}\nm{\oint_C\frac{f(z)\bigl((z-z_0)^2-(z-z_0)(z-z_0-\Delta z)-\Delta z(z-z_0-\Delta z)\bigr)}{\Delta z(z-z_0-\Delta z)(z-z_0)^2}\,\dz}\\
% &=\frac 1{2\pi}\nm{\oint_C\frac{f(z)\Delta z}{(z-z_0-\Delta z)(z-z_0)^2}\,\dz}\le \frac{ML}{2\pi(\delta-\Delta z)\delta^2}\nm{\Delta z}
% \end{aligned}
% \end{gather*}
% where $M$ is an upper bound for $\nm{f(z)}$ on $C$, and $L$ is the length of $C$. Clearly this goes to zero as $\Delta z\to 0$: we conclude that $f'(z_0)=\frac 1{2\pi i}\oint_C\frac{f(z)}{(z-z_0)^2}\,\dz$.\\
% One can repeat this exercise for higher derivatives: for instance,
% \begin{gather*}
% \nm{\frac{f'(z_0+\Delta z)-f'(z_0)}{\Delta z}-\frac 2{2\pi i}\oint_C\frac{f(z)}{(z-z_0)^3}\,\dz}\\
% \begin{aligned}
% \qquad\qquad&=\nm{\frac 1{2\pi i}\oint_C\frac{f(z)}{\Delta z}\left(\frac 1{(z-z_0-\Delta z)^2}-\frac 1{(z-z_0)^2}\right)-\frac{2f(z)}{(z-z_0)^3}\,\dz}\\
% &=\frac 1{2\pi}\nm{\oint_C\frac{\bigl(3(z-z_0)-2\Delta z)f(z)\Delta z}{(z-z_0-\Delta z)^2(z-z_0)^3}\,\dz}\le \frac{(3K+2\delta)ML}{2\pi(\delta-\Delta z)^2\delta^3}\nm{\Delta z}
% \end{aligned}
% \end{gather*}
% where $\nm{z-z_0}\le K$ is the largest distance from $z_0$ to a point on $C$. Again this tends to zero as $\Delta z\to 0$.\\[5pt] 
% The nastier calculations for the higher derivatives are omitted for sanity's sake\ldots
\clearpage
...
% \begin{proof}
% It costs nothing to assume that the polynomial is monic. Suppose $p(z)=z^n+\sum_{k=0}^{n-1}a_kz^k$ has no roots, then $\frac 1{p(z)}$ is entire. Our goal is to prove that $\frac 1{p(z)}$ is also \emph{bounded} on $\C$. Since  $p(z)$ is not constant, this contradicts Liouville's Theorem.\smallbreak
% For each $k\le n-1$, define $R_k:=(2n\nm{a_k})^{\frac 1{n-k}}\ge 0$ and set $R:=\max\{R_0,\ldots,R_{n-1}\}$. Then, if $a_k\neq 0$,
% \[\nm z>R\implies \frac{\nm{a_k}}{\nm z^{n-k}}<\frac{\nm{a_k}}{R^{n-k}}\le \frac{\nm{a_k}}{R_k^{n-k}}=\frac 1{2n}\]
% The inequality is trivially true if $\nm{a_k}=0$. But then
% \begin{align*}
% \nm z>R&\implies\nm{\sum_{k=0}^{n-1}\frac{a_k}{z^{n-k}}}<\frac 12\\
% &\implies \nm{p(z)}=\nm z^n\nm{1+\sum_{k=0}^{n-1}\frac{a_k}{z^{n-k}}}  \ge\frac 12\nm z^n>\frac 12R^n
% \end{align*}
% whence $\frac 1{\nm{p(z)}}<\frac 2{R^n}$ is bounded whenever $\nm z>R$. Since $\frac 1{p(z)}$ is continuous, it is also bounded on the closed disk $\nm z\le R$, and thus on $\C$. We have our contradiction.
% \end{proof}
...
\begin{examples}{}{modulusexplicit}
\hangindent\leftmargini
...
% \smallbreak
%   Evaluate using polar co-ordinates and apply the usual method from multi-variable calculus:
%   \[\nm{f(r,\theta)}^2=\nm{r^2e^{2i\theta}-ire^{i\theta}}^2=r^2(r^2+1-2r\sin\theta)\]
%   If $(r,\theta)$ is a maximum of $\nm{f(r,\theta)}^2$ inside the domain ($0<r<1$ and $0<\theta<\pi$) then its partial derivatives are zero:
%   \[\partials\theta\nm{f(r,\theta)}^2=-2r^3\cos\theta=0\iff \theta=\frac\pi 2\]
%   Only the latter matters, since $r=0$ is on the edge of the domain: for the other derivative
%   \[\partialsat r{(r,\frac\pi 2)}\nm{f(r,\theta)}^2=2r(2r^2+1-3r\sin\theta)\big|_{(r,\frac\pi 2)}=2r(2r-1)(r-1)=0\iff r=\frac 12\]
%   Indeed $f(\frac 12,\frac\pi 2)=f(\frac i2)=\frac 14$. On the boundary:
%   \begin{itemize}
%     \item $\theta=0,\pi\implies \nm{f(r,\theta)}^2=r^2(r^2+1)$ is maximal when $r=1$.
%     \item $r=1\implies \nm{f(r,\theta)}^2=2-2\sin\theta$ is maximal when $\theta=\frac\pi 2$.
%   \end{itemize}
%   The maximum value is therefore at $z=i$ which yields $f(z)=-1+1=0$. Both of these conditions lie on the edge of the domain.
\end{examples}
...
\footnotetext{Recall from chapter 2: $\nm{f(z)}=k\neq 0\implies \cl{f(z)}=\frac{k^2}{f(z)}$ is holomorphic, whence $u-iv$ satisfies the Cauchy--Riemann equations\ldots}
\pagebreak
\goodbreak
\begin{tcolorbox}[proofstyle]
...
% \begin{proof}
% Let $r\le\delta$ and apply the Cauchy integral formula around the circle \textcolor{brown}{$C_{0,r}$} of radius $r$ centered at $z_0$:
% \begin{align*}
% f(z_0)&=\frac 1{2\pi i}\oint_{C_{0,r}}\frac{f(z)}{z-z_0}\,\dz =\frac 1{2\pi i}\int_0^{2\pi}\frac{f(z_0+r e^{it})}{r e^{it}}ir e^{it}\,\dt =\frac 1{2\pi}\int_0^{2\pi}f(z_0+r e^{it})\,\dt\\
% \implies \nm{f(z_0)}&\le \frac 1{2\pi}\int_0^{2\pi}\nm{f(z_0+r e^{it})}\,\dt \tag{Lemma \ref{lemm:intbound}}\\
% &\le \frac 1{2\pi}\int_0^{2\pi}\nm{f(z_0)}\,\dt =\nm{f(z_0)} \tag{$\nm{f(z_0)}=\max\{\nm{f(z)}:z\in D\}$}
% \end{align*}
% It follows that the inequalities are \emph{equalities,} and so
% \[\nm{f(z_0)}=\frac 1{2\pi}\int_0^{2\pi}\nm{f(z_0+r e^{it})}\,\dt
% \implies \int_0^{2\pi}\nm{f(z_0)}-\nm{f(z_0+r e^{it})}\,\dt=0\]
% Since the integrand is non-negative and continuous, it must be zero. But then $\nm{f(z)}=\nm{f(z_0)}$ on \emph{all circles} of radius $\le\delta$ centered at $z_0$, whence $\nm{f(z)}$ is  \emph{constant} on $B_0$. Since $f(z)$ is holomorphic, it is also constant\footnotemark on $B_0$.\smallbreak
% To finish the proof, note that $z_1\in B_0\cap B_1\implies f(z_1)=f(z_0)$ and so $\nm{f(z)}\le \nm{f(z_1)}$ on $B_1$. Thus $f(z)=f(z_0)$ on $B_1$ also. Repeating this process $n$ times, we see that $f(z)=f(z_0)$ on every disk $B_k$: in particular, $w\in B_n$ and so $f(w)=f(z_0)$.
% \end{proof}
