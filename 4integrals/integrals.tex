...
\begin{exercises}
	...
	\begin{enumerate}\setcounter{enumi}{1}
	  ...
	  \item\label{exs:1-zsq}\begin{enumerate}
	    \item Find an anti-derivative of $\frac 1{1-z^2}$ on the domain $D=\C$ except for the real axis where $\nm z\ge 1$. Evaluate $\int_0^{1+2i}\frac 1{1-z^2}\,\dz$ along any curve in $D$.
	    	...
	    	\item (Hard)\lstsp Prove directly that $\oint_{C_r}\frac 1{1-z^2}\,\dz =-i\pi$ where $C_r$ is the circle of radius $r<2$ centered at $z=1$.\par
...
\begin{examples}{}{intformula}
	...
	\begin{enumerate}
  	...
  	\]
%   	Suppose we didn't have the integral formula and instead chose to parametrize directly via $z(t)=1+e^{it}$: we'd have to compute the integral
%   	\[
%   		\oint_C \frac 1{1-z^2}\,\dz =\int_0^{2\pi} \frac{ie^it}{-e^{it}(2+e^{it})}\,\dt
%   		=-i\int_0^{2\pi} \frac{\dt}{2+e^{it}}\,\dt
%   	\]
